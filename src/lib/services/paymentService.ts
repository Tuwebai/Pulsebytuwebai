...
import { MERCADOPAGO_CONFIG, PAYMENT_TYPES, PAYMENT_STATUS, formatCurrency, toCents } from '../integrations/mercadopago';
...
export const createMercadoPagoPreference = async (paymentData: CreatePaymentData) => {
  try {
    ...
    if (!paymentType) {
      throw new Error('Tipo de pago no vÃ¡lido');
    }
    ...
    // En producciÃ³n, esto se harÃ­a desde el backend
...
export const processMercadoPagoWebhook = async (webhookData: any) => {
  try {
    ...
    if (type === 'payment') {
      ...
      // Obtener informaciÃ³n del pago desde Mercado Pago
...
const getMercadoPagoPayment = async (paymentId: string) => {
  // En producciÃ³n, esto se harÃ­a con la API de Mercado Pago
        ...
        supabase
          ...
          .then(({ data, error }) => {
            if (!error && data) {
              ...
              // En caso de error, llamar callback con array vacÃ­o
              callback([]);
            }
          });
      }
  ...
  supabase
    ...
    .then(({ data, error }) => {
      if (!error && data) {
        ...
        // Si no hay datos, llamar callback con array vacÃ­o
        callback([]);
      }
    });
...
// FunciÃ³n auxiliar para convertir de centavos
