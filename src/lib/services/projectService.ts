...
export const projectService = {
  ...
  /**
   * Obtener todos los proyectos con filtros, ordenamiento y paginaciÃ³n
   */
  ...
  /**
   * Crear un nuevo proyecto con sistema de aprobaciÃ³n
   */
  async createProject(projectData: CreateProjectData & { created_by?: string, user_role?: string }): Promise<Project> {
    try {
      ...
      if (!validation.isValid) {
        throw new Error(`Datos del proyecto invÃ¡lidos: ${validation.errors.join(', ')}`);
      }

      // VALIDACIÃ“N CRÃTICA: Asegurar que created_by estÃ© presente y sea vÃ¡lido
      if (!projectData.created_by || projectData.created_by.trim() === '') {
        throw new Error('El campo created_by es requerido y debe ser un ID de usuario vÃ¡lido');
      }
      ...
      if (userError) {
        console.error('âŒ Error obteniendo rol del usuario:', userError);
        throw new Error('No se pudo verificar el rol del usuario');
      }

      // Determinar el estado de aprobaciÃ³n basado en el rol del usuario
      ...
      // Crear proyecto con estado de aprobaciÃ³n segÃºn el rol
      const projectToCreate = {
        ...
        type: detectedType.name, // Asignar tipo detectado automÃ¡ticamente
        // Si es admin, marcar como aprobado por Ã©l mismo
        ...(userData.role === 'admin' && {
      ...
      if (error) {
        console.error('âŒ Error de Supabase al crear proyecto:', error);
        
        // Manejar errores especÃ­ficos de Supabase
        if (error.code === '23505') {
          throw new Error('Ya existe un proyecto con ese nombre');
        } else if (error.code === '23503') {
          throw new Error('Usuario no vÃ¡lido para crear el proyecto');
  ...
  /**
   * Obtener proyectos por tecnologÃ­as
   */
  async getProjectsByTechnologies(technologies: string[]): Promise<Project[]> {
    try {
      ...
      throw new Error(`Error al obtener proyectos por tecnologÃ­as: ${error.message}`);
    }
  },
  ...
  async deleteProject(projectId: string, userId: string, userRole?: string): Promise<void> {
    try {
      ...
      if (fetchError) {
        console.error('âŒ Error obteniendo proyecto:', fetchError);
      ...
      if (role === 'admin') {
        ...
        if (error) {
          console.error('âŒ Error eliminando proyecto (admin):', error);
        ...
        if (error) {
          console.error('âŒ Error eliminando proyecto (cliente):', error);
          throw new Error('Error al eliminar el proyecto');
        }
      }
    } catch (error) {
  ...
  /**
   * Obtener estadÃ­sticas generales de proyectos
   */
  ...
  /**
   * Obtener tecnologÃ­as Ãºnicas con conteo
   */
  async getUniqueTechnologies(): Promise<Array<{ technology: string; count: number }>> {
    try {
      ...
      throw new Error(`Error al obtener tecnologÃ­as: ${error.message}`);
    }
  },
  ...
  validateProjectData(data: CreateProjectData): { isValid: boolean; errors: string[] } {
    ...
    if (data.description && data.description.length > 10000) {
      errors.push('La descripciÃ³n no puede exceder 10,000 caracteres');
    }

    if (data.technologies && data.technologies.length > 20) {
      errors.push('No se pueden agregar mÃ¡s de 20 tecnologÃ­as');
    }

    if (data.github_repository_url && !this.validateGitHubUrl(data.github_repository_url)) {
      errors.push('La URL de GitHub no es vÃ¡lida');
    }
  ...
  /**
   * Obtener proyectos similares basados en tecnologÃ­as
   */
  ...
  // =====================================================
  // FUNCIONES DE APROBACIÃ“N DE PROYECTOS
  // =====================================================

  /**
   * Obtener solicitudes de aprobaciÃ³n pendientes (solo para admins)
   */
  async getApprovalRequests() {
    try {
      ...
      throw new Error(`Error al obtener solicitudes de aprobaciÃ³n: ${error.message}`);
    }
  },
  ...
  /**
   * Crear una solicitud de aprobaciÃ³n manual (para proyectos rechazados)
   */
  async createApprovalRequest(projectId: string, requestNotes?: string): Promise<string> {
    try {
      ...
      throw new Error(`Error al crear solicitud de aprobaciÃ³n: ${error.message}`);
    }
  }
};
