...
export class WorkflowService {
  ...
  /**
   * Ejecutar paso especÃ­fico de workflow
   */
  ...
  private async executeNotificationAction(actionData: any, projectId: string): Promise<void> {
    try {
      ...
      // Ejemplo: crear notificaciÃ³n en la base de datos
      const { error } = await supabase
        .from('notifications')
        .insert({
          title: actionData.title || 'NotificaciÃ³n del Workflow',
          message: actionData.message || 'AcciÃ³n automÃ¡tica ejecutada',
          type: 'info',
          category: 'workflow',
  ...
  /**
   * Completar ejecuciÃ³n de workflow
   */
  ...
  private async recordStepResult(executionId: string, stepId: string, status: string, executionTime: number, errorMessage?: string): Promise<void> {
    try {
      // Actualizar ejecuciÃ³n con resultado del paso
      const { error } = await supabase
        .from('workflow_executions')
        .update({
  ...
  private async moveToNextStep(executionId: string, currentStep: WorkflowStep, projectId: string): Promise<void> {
    try {
      ...
      if (nextStep) {
        // Actualizar ejecuciÃ³n con siguiente paso
        const { error } = await supabase
          .from('workflow_executions')
  ...
  // =====================================================
  // MÃ‰TODOS PARA VERSION MANAGEMENT
  // =====================================================

  /**
   * Transicionar estado de versiÃ³n
   */
  async transitionVersionStatus(
    ...
    try {
      ...
      await this.logWorkflowAction('version', versionId, 'status_transition', 'error', 'Error en transiciÃ³n de estado', { 
        newStatus, 
        userId, 
        comments,
        error: error.message 
  ...
  async triggerAutoRollback(versionId: string, comments?: string): Promise<boolean> {
    try {
      ...
      // Log del rollback
      await this.logWorkflowAction('version', versionId, 'rollback', 'success', 'Rollback ejecutado automÃ¡ticamente', { 
        comments,
      ...
      await this.logWorkflowAction('version', versionId, 'rollback', 'error', 'Error en rollback automÃ¡tico', { 
        comments,
        error: error.message 
      });
      return false;
  ...
  async detectNewCommits(projectId: string): Promise<{ newCommits: any[]; hasNewCommits: boolean }> {
    try {
      // Simular detecciÃ³n de commits nuevos
  ...
  async createVersionFromCommits(
    ...
    try {
      ...
      const newVersion = {
        ...
        changes: [
          { type: 'feature', title: 'Auto-generated from commits', description: 'VersiÃ³n creada automÃ¡ticamente', author: userId }
        ]
      };

      // Log de la creaciÃ³n
      await this.logWorkflowAction('project', projectId, 'create_version', 'success', `VersiÃ³n ${version} creada automÃ¡ticamente`, { 
        version, 
        environment, 
        userId 
      });
      ...
      await this.logWorkflowAction('project', projectId, 'create_version', 'error', 'Error creando versiÃ³n automÃ¡tica', { 
        version, 
        environment, 
        userId,
        error: error.message 
  ...
  /**
   * Validar versiÃ³n semÃ¡ntica
   */
  validateSemanticVersion(version: string): { isValid: boolean; errors: string[] } {
    try {
      const errors: string[] = [];
      
      // Validar formato semÃ¡ntico (x.y.z)
      const semanticRegex = /^\d+\.\d+\.\d+$/;
      if (!semanticRegex.test(version)) {
        errors.push('Formato de versiÃ³n invÃ¡lido. Debe ser x.y.z (ej: 1.0.0)');
      }
      ...
      for (const part of parts) {
        if (isNaN(Number(part)) || Number(part) < 0) {
          errors.push('Los nÃºmeros de versiÃ³n deben ser enteros positivos');
          break;
        }
      }
      ...
      return {
        isValid: false,
        errors: ['Error validando versiÃ³n semÃ¡ntica']
      };
    }
  }
}
