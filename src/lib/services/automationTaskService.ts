...
export class AutomationTaskService {
  ...
  async executeTask(taskId: string): Promise<TaskExecutionResult> {
    try {
      ...
      if (!task.is_active) {
        throw new Error('Tarea no estÃ¡ activa');
      }


      const startTime = Date.now();

      // Ejecutar script segÃºn el tipo
      let output: any;
      let success = false;
    ...
    try {
      ...
      for (const task of tasksToExecute) {
        try {
          ...
          console.error(`âŒ Error ejecutando tarea ${task.id}:`, error);
  ...
  private async executeSqlScript(script: string, parameters: Record<string, any>): Promise<any> {
    try {
      ...
      // Reemplazar parÃ¡metros en el script
  ...
  private async executeJavaScriptScript(script: string, parameters: Record<string, any>): Promise<any> {
    try {
      ...
      // Crear funciÃ³n segura para ejecutar el script
      const safeFunction = new Function('parameters', 'supabase', script);
      
      // Ejecutar script con parÃ¡metros y acceso a Supabase
  ...
  private async executeShellScript(script: string, parameters: Record<string, any>): Promise<any> {
    try {
      ...
      // AquÃ­ podrÃ­as implementar una simulaciÃ³n o usar un servicio backend
      console.warn('âš ï¸ Scripts shell no se pueden ejecutar en el frontend');
      
      // Por ahora, simulamos la ejecuciÃ³n
      const simulatedOutput = `SimulaciÃ³n de ejecuciÃ³n de script shell: ${script}`;
  ...
  /**
   * Programar prÃ³xima ejecuciÃ³n de la tarea
   */
    ...
    try {
      ...
      switch (taskType) {
        ...
        case 'monthly':
          ...
          break;
        case 'on_event':
          // Las tareas de evento no se programan automÃ¡ticamente
          return;
        case 'custom':
          // Las tareas personalizadas mantienen su programaciÃ³n
          return;
        default:
  ...
  /**
   * Programar tarea para ejecutarse en un momento especÃ­fico
   */
  ...
  async validateScript(script: string, scriptType: string): Promise<{ isValid: boolean; errors: string[] }> {
    ...
    try {
      switch (scriptType) {
        case 'sql':
          // ValidaciÃ³n bÃ¡sica de SQL
          if (!script.trim()) {
            errors.push('El script SQL no puede estar vacÃ­o');
          }
        ...
        case 'javascript':
          // ValidaciÃ³n bÃ¡sica de JavaScript
          try {
        ...
        case 'shell':
          ...
          errors.push('Los scripts shell no estÃ¡n soportados en el frontend');
          break;
      ...
      errors.push(`Error de validaciÃ³n: ${error.message}`);
    }
  ...
  /**
   * Obtener estadÃ­sticas de tareas automatizadas
   */
  ...
  getTaskTemplates(): Record<string, CreateAutomationTaskDto> {
    return {
      'backup_database': {
        ...
        description: 'Crear backup automÃ¡tico de la base de datos',
        type: 'daily',
      ...
      'generate_reports': {
        ...
        description: 'Generar reportes automÃ¡ticos del sistema',
        type: 'monthly',
