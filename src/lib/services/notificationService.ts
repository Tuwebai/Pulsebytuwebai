...
class NotificationService {
  ...
  async getUserNotifications(filters: NotificationFilters = {}): Promise<Notification[]> {
    try {
      ...
      // FILTRO CRÃTICO: Excluir notificaciones donde el sender_id sea igual al currentUserId
      // Nota: No podemos usar neq con metadata->sender_id en Supabase, lo filtramos despuÃ©s
      ...
      if (error) {
        // No loggear el error aquÃ­, se manejarÃ¡ en el catch
        throw error;
      }
  ...
  // M?todo de compatibilidad para flujos legacy de notificaciones
  async sendNotification(payload: SendNotificationPayload): Promise<Notification[]> {
    ...
    return this.createNotificationForUsers(recipients, {
      title,
      message,
      type,
      category,
  ...
  // Crear notificaciÃ³n para mÃºltiples usuarios
  ...
  // Crear notificaciÃ³n para todos los usuarios
  ...
  // Crear notificaciones automÃ¡ticas para eventos del sistema
  ...
  // Crear notificaciÃ³n de proyecto
  ...
  // Crear notificaciÃ³n de seguridad
