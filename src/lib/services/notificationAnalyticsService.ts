...
export class NotificationAnalyticsService {
  ...
  /**
   * Obtener insights y recomendaciones basados en analytics
   */
  async getNotificationInsights(
    filters?: { startDate?: string; endDate?: string }
  ): Promise<NotificationInsights> {
    try {
      const [channels, categories, timeSeries] = await Promise.all([
        this.getChannelPerformance(filters?.startDate, filters?.endDate),
        this.getCategoryPerformance(filters?.startDate, filters?.endDate),
        this.getTimeSeriesData(30)
      ]);

      // Encontrar mejor canal
      const bestChannel = channels.reduce((best, current) => 
        current.deliveryRate > (best?.deliveryRate || 0) ? current : best
      , channels[0]);
      ...
      // Analizar tendencias (comparando primera mitad vs segunda mitad del período)
      ...
      const firstHalfDelivery = firstHalf.reduce((sum, d) => sum + d.delivered, 0);
      const firstHalfSent = firstHalf.reduce((sum, d) => sum + d.sent, 0);
      const secondHalfDelivery = secondHalf.reduce((sum, d) => sum + d.delivered, 0);
      const secondHalfSent = secondHalf.reduce((sum, d) => sum + d.sent, 0);

      const firstHalfRate = firstHalfSent > 0 ? (firstHalfDelivery / firstHalfSent) * 100 : 0;
      const secondHalfRate = secondHalfSent > 0 ? (secondHalfDelivery / secondHalfSent) * 100 : 0;

      let deliveryTrend: 'increasing' | 'decreasing' | 'stable' = 'stable';
      if (secondHalfRate > firstHalfRate + 2) deliveryTrend = 'increasing';
      else if (secondHalfRate < firstHalfRate - 2) deliveryTrend = 'decreasing';

      // Análisis de apertura
      const firstHalfOpened = firstHalf.reduce((sum, d) => sum + d.opened, 0);
      const secondHalfOpened = secondHalf.reduce((sum, d) => sum + d.opened, 0);

      const firstHalfOpenRate = firstHalfDelivery > 0 ? (firstHalfOpened / firstHalfDelivery) * 100 : 0;
      const secondHalfOpenRate = secondHalfDelivery > 0 ? (secondHalfOpened / secondHalfDelivery) * 100 : 0;

      let openTrend: 'increasing' | 'decreasing' | 'stable' = 'stable';
      if (secondHalfOpenRate > firstHalfOpenRate + 2) openTrend = 'increasing';
      else if (secondHalfOpenRate < firstHalfOpenRate - 2) openTrend = 'decreasing';

      // Determinar hora pico (basado en datos disponibles)
      const peakHour = '09:00'; // Default, podría calcularse si hay datos por hora

      // Generar recomendaciones
      const recommendations: string[] = [];

      if (bestChannel && bestChannel.deliveryRate < 80) {
        recommendations.push(`Considera optimizar el canal "${bestChannel.channel}" - tasa de entrega actual: ${bestChannel.deliveryRate.toFixed(1)}%`);
      }

      if (openTrend === 'decreasing') {
        recommendations.push('La tasa de apertura está disminuyendo. Considera revisar el contenido y timing de tus notificaciones.');
      }

      if (deliveryTrend === 'decreasing') {
        recommendations.push('La tasa de entrega está disminuyendo. Verifica la reputación del remitente y lista de destinatarios.');
      }

      if (recommendations.length === 0) {
        recommendations.push('¡Excelente! Tus métricas de notificaciones están funcionando correctamente.');
      }

      return {
        bestPerformingChannel: bestChannel?.channel || 'N/A',
        bestPerformingCategory: bestCategory?.category || 'N/A',
        peakSendingTime: peakHour,
        trends: {
      ...
      return {
        ...
        recommendations: ['No se pudieron generar insights. Verifica la conexión con la base de datos.']
      };
    }
  }
