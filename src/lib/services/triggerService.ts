...
export class TriggerService {
  ...
  async fireTrigger(eventType: SystemTrigger['event_type'], eventData: any): Promise<void> {
    try {
      // console.log(`ðŸ”¥ Disparando triggers para evento: ${eventType}`);
      
      // Obtener triggers activos para este tipo de evento
      const triggers = await this.getTriggersByEventType(eventType);
      
      if (triggers.length === 0) {
        // console.log(`â„¹ï¸ No hay triggers activos para el evento: ${eventType}`);
        return;
      }

      // console.log(`ðŸ“‹ Encontrados ${triggers.length} triggers para ejecutar`);
      ...
      for (const trigger of triggers) {
        try {
          ...
          console.error(`âŒ Error ejecutando trigger ${trigger.id}:`, error);
  ...
  /**
   * Ejecutar trigger especÃ­fico
   */
  async executeTrigger(trigger: SystemTrigger, eventData: any): Promise<void> {
    try {
      const startTime = Date.now();
      // console.log(`âš¡ Ejecutando trigger: ${trigger.name}`);

      // Verificar si el trigger debe ejecutarse segÃºn el schedule
      if (trigger.schedule && !this.shouldExecuteBySchedule(trigger.schedule)) {
        // console.log(`â° Trigger ${trigger.name} no debe ejecutarse segÃºn su schedule`);
        return;
      }
      ...
      if (trigger.conditions && Object.keys(trigger.conditions).length > 0) {
        ...
        if (!conditionsMet) {
          // console.log(`âŒ Condiciones no cumplidas para trigger ${trigger.name}`);
      ...
      // Actualizar estadÃ­sticas del trigger
      await this.updateTriggerStats(trigger.id);

      const executionTime = Date.now() - startTime;
      // console.log(`âœ… Trigger ${trigger.name} ejecutado exitosamente en ${executionTime}ms`);
      ...
      console.error(`âŒ Error ejecutando trigger ${trigger.name}:`, error);
      await this.logTriggerAction('trigger', trigger.id, 'execute', 'error', `Error ejecutando trigger: ${error.message}`);
      throw error;
    }
  }
  ...
  /**
   * Verificar si el trigger debe ejecutarse segÃºn su schedule
   */
  private shouldExecuteBySchedule(schedule: string): boolean {
    try {
      // ImplementaciÃ³n bÃ¡sica de cron parsing
      // Por ahora, solo verificamos si es un schedule vÃ¡lido
      if (!schedule || schedule.trim() === '') return true;
      
      // AquÃ­ podrÃ­as implementar un parser de cron mÃ¡s robusto
      ...
      for (const [actionType, actionData] of Object.entries(actions)) {
        switch (actionType) {
          ...
          case 'slack_notification':
            ...
            break;
          default:
            console.warn(`AcciÃ³n no soportada: ${actionType}`);
        }
      }
    } catch (error) {
  ...
  // =====================================================
  // ACCIONES ESPECÃFICAS DE TRIGGERS
  // =====================================================

  /**
   * Ejecutar acciÃ³n de notificaciÃ³n
   */
  private async executeNotificationAction(actionData: any, eventData: any): Promise<void> {
    try {
      // console.log('ðŸ“¢ Ejecutando acciÃ³n de notificaciÃ³n:', actionData);
  ...
  /**
   * Ejecutar acciÃ³n de email
   */
  private async executeEmailAction(actionData: any, eventData: any): Promise<void> {
    try {
      // console.log('ðŸ“§ Ejecutando acciÃ³n de email:', actionData);
      ...
      // console.log('ðŸ“§ Datos de email preparados:', emailData);
      
      // Por ahora solo log, pero aquÃ­ enviarÃ­as el email
  ...
  /**
   * Ejecutar acciÃ³n de actualizaciÃ³n de estado
   */
  private async executeStatusUpdateAction(actionData: any, eventData: any): Promise<void> {
    try {
      // console.log('ðŸ”„ Ejecutando acciÃ³n de actualizaciÃ³n de estado:', actionData);
      ...
      if (!table || !recordId || !newStatus) {
        throw new Error('Faltan campos requeridos para actualizaciÃ³n de estado');
      }
      ...
      // console.log(`âœ… Estado actualizado en ${table} para ID ${recordId}`);
    } catch (error) {
  ...
  /**
   * Ejecutar acciÃ³n de crear ticket
   */
  private async executeCreateTicketAction(actionData: any, eventData: any): Promise<void> {
    try {
      // console.log('ðŸŽ« Ejecutando acciÃ³n de crear ticket:', actionData);
  ...
  /**
   * Ejecutar acciÃ³n de asignaciÃ³n
   */
  private async executeAssignmentAction(actionData: any, eventData: any): Promise<void> {
    try {
      // console.log('ðŸ‘¤ Ejecutando acciÃ³n de asignaciÃ³n:', actionData);
      ...
      if (!table || !recordId || !userId) {
        throw new Error('Faltan campos requeridos para asignaciÃ³n');
      }
      ...
      // console.log(`âœ… Usuario ${userId} asignado a ${table} ID ${recordId}`);
    } catch (error) {
  ...
  /**
   * Ejecutar acciÃ³n de webhook
   */
  private async executeWebhookAction(actionData: any, eventData: any): Promise<void> {
    try {
      // console.log('ðŸŒ Ejecutando acciÃ³n de webhook:', actionData);
      ...
      if (!response.ok) {
        throw new Error(`Webhook fallÃ³ con status: ${response.status}`);
      }
      
      // console.log('âœ… Webhook ejecutado exitosamente');
    } catch (error) {
  ...
  /**
   * Obtener estadÃ­sticas de triggers
   */
