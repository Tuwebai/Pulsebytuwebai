...
export class ScheduledNotificationService {
  ...
  /**
   * Programar una notificaciÃ³n para envÃ­o futuro
   */
  ...
  /**
   * Programar notificaciÃ³n usando plantilla
   */
  ...
  /**
   * ProgramaciÃ³n inteligente basada en preferencias del usuario
   */
  async smartSchedule(options: SmartScheduleOptions): Promise<ScheduledNotification> {
    try {
      ...
      // Aplicar filtros de canales segÃºn preferencias
      const template = await notificationTemplateService.getTemplate(templateId);
  ...
  /**
   * Iniciar procesamiento automÃ¡tico de notificaciones programadas
   */
  ...
  async processScheduledNotifications(): Promise<void> {
    try {
      ...
      const { data: readyNotifications, error } = await supabase
        ...
        .limit(50); // Procesar mÃ¡ximo 50 a la vez
  ...
  private calculateBestSendTime(preferences?: SmartScheduleOptions['preferences']): string {
    ...
    if (currentTime >= businessStart && currentTime <= businessEnd) {
      ...
      // Estamos fuera de horario, enviar al inicio del prÃ³ximo dÃ­a laboral
  ...
  /**
   * Filtrar canales segÃºn preferencias del usuario
   */
  ...
  /**
   * Obtener estadÃ­sticas de notificaciones programadas
   */
