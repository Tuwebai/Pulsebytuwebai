...
// =====================================================
// SERVICIO DE PLANTILLAS DE NOTIFICACIÃ“N
// =====================================================
...
export class NotificationTemplateService {
  private defaultTemplates: NotificationTemplate[] = [
    {
      ...
      content: 'Hola {{user.fullName}}, nos alegra que te hayas unido a Pulse by TuWebAI. Tu cuenta ha sido creada exitosamente y ya puedes comenzar a usar todas nuestras funcionalidades.',
      htmlContent: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <h1 style="color: #2563eb;">Bienvenido a Pulse by TuWebAI</h1>
          <p>Hola <strong>{{user.fullName}}</strong>,</p>
          <p>Nos alegra que te hayas unido a Pulse by TuWebAI. Tu cuenta ha sido creada exitosamente y ya puedes comenzar a usar todas nuestras funcionalidades.</p>
          ...
          <p>Saludos,<br>El equipo de Pulse by TuWebAI</p>
        </div>
      `,
      variables: {
        user: {
    ...
    },
    {
      id: 'project-update',
      name: 'project-update',
      displayName: 'ActualizaciÃ³n de Proyecto',
      ...
      htmlContent: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <h2 style="color: #059669;">ActualizaciÃ³n de Proyecto</h2>
          <div style="background-color: #f0fdf4; padding: 20px; border-radius: 8px; border-left: 4px solid #059669;">
            <h3>{{project.name}}</h3>
            <p><strong>AcciÃ³n:</strong> {{update.action}}</p>
            <p><strong>DescripciÃ³n:</strong> {{update.description}}</p>
            <p><strong>Fecha:</strong> {{update.date}}</p>
          </div>
          <p>Para mÃ¡s detalles, visita tu dashboard.</p>
        </div>
      `,
      variables: {
        project: {
        ...
        update: {
          ...
          date: { type: 'date', required: true, description: 'Fecha de la actualizaciÃ³n' }
        }
      },
      isActive: true,
      createdAt: new Date().toISOString(),
    ...
    },
    {
      ...
      description: 'Plantilla para notificar asignaciÃ³n de tickets',
      category: 'ticket',
      channels: ['email', 'push', 'in_app'],
      ...
      htmlContent: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <div style="background-color: #fef2f2; padding: 20px; border-radius: 8px; border-left: 4px solid #dc2626;">
            ...
            <p><strong>DescripciÃ³n:</strong> {{ticket.description}}</p>
            <p><strong>Asignado por:</strong> {{ticket.assignedBy}}</p>
            <p><strong>Fecha lÃ­mite:</strong> {{ticket.dueDate}}</p>
          </div>
          <p>Accede al ticket para comenzar a trabajar en Ã©l.</p>
        </div>
      `,
      variables: {
        ticket: {
          id: { type: 'string', required: true, description: 'ID del ticket' },
          title: { type: 'string', required: true, description: 'TÃ­tulo del ticket' },
          ...
          description: { type: 'string', required: true, description: 'DescripciÃ³n del ticket' },
          assignedBy: { type: 'string', required: true, description: 'Usuario que asignÃ³ el ticket' },
          dueDate: { type: 'date', required: false, description: 'Fecha lÃ­mite del ticket' }
        }
      },
      isActive: true,
      createdAt: new Date().toISOString(),
    ...
    },
    {
      ...
      variables: {
        payment: {
          ...
          method: { type: 'string', required: true, description: 'MÃ©todo de pago' },
    ...
    },
    {
      ...
      subject: 'ðŸš¨ Alerta de Seguridad - {{alert.type}}',
      content: 'Se ha detectado una actividad de seguridad: {{alert.description}}. IP: {{alert.ipAddress}}, Fecha: {{alert.date}}',
      htmlContent: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <h2 style="color: #dc2626;">ðŸš¨ Alerta de Seguridad</h2>
          <div style="background-color: #fef2f2; padding: 20px; border-radius: 8px; border-left: 4px solid #dc2626;">
            <h3>{{alert.type}}</h3>
            <p><strong>DescripciÃ³n:</strong> {{alert.description}}</p>
            <p><strong>IP:</strong> {{alert.ipAddress}}</p>
            <p><strong>Fecha:</strong> {{alert.date}}</p>
            <p><strong>UbicaciÃ³n:</strong> {{alert.location}}</p>
          </div>
      ...
      variables: {
        alert: {
          ...
          location: { type: 'string', required: false, description: 'UbicaciÃ³n geogrÃ¡fica' }
        }
      },
      isActive: true,
      createdAt: new Date().toISOString(),
  ...
  // =====================================================
  // INICIALIZACIÃ“N DE PLANTILLAS POR DEFECTO
  // =====================================================
  ...
  private getCategoryDescription(category: string): string {
    const descriptions: Record<string, string> = {
      ...
      payment: 'Notificaciones de pagos y facturaciÃ³n',
      security: 'Alertas de seguridad y autenticaciÃ³n',
