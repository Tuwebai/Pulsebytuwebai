...
export const handleMercadoPagoWebhook = async (webhookData: MercadoPagoWebhook) => {
  try {
    ...
    // Registrar el webhook en Supabase para auditorÃ­a
...
// FunciÃ³n para sincronizar pagos desde la pÃ¡gina principal
...
// FunciÃ³n para verificar estado de pagos pendientes
  ...
  try {
    ...
    for (const payment of pendingPayments) {
      if (payment.mercadopagoId) {
        try {
          // En producciÃ³n, aquÃ­ harÃ­as una llamada a la API de Mercado Pago
...
// FunciÃ³n para verificar estado de pago en Mercado Pago
const checkMercadoPagoPaymentStatus = async (paymentId: string) => {
  // En producciÃ³n, aquÃ­ harÃ­as una llamada real a la API de Mercado Pago
