...
export interface EscalationActionConfig {
  ...
  type: 'notify' | 'assign' | 'update_stage' | 'update_priority' | 'create_task' | 'send_email';
  parameters: Record<string, any>;
  delay?: number; // retraso especÃ­fico para esta acciÃ³n
  retryCount?: number;
  retryDelay?: number;
}
...
class TicketEscalationService {
  ...
  // Iniciar monitoreo de escalaciÃ³n
  ...
  // Verificar escalaciÃ³n de un ticket especÃ­fico
  private async checkTicketEscalation(ticketData: TicketData): Promise<void> {
    try {
      // Ordenar reglas por prioridad y nivel de escalaciÃ³n
      ...
      for (const rule of sortedRules) {
        ...
        if (shouldEscalate) {
          // Verificar si ya existe una escalaciÃ³n activa para este ticket
  ...
  private async executeEscalation(ticketData: TicketData, rule: EscalationRule): Promise<void> {
    try {
      ...
      // Ejecutar acciones de escalaciÃ³n
  ...
  private async executeEscalationAction(
    ...
    try {
      ...
      // Agregar acciÃ³n a la escalaciÃ³n
      const { data: escalationData, error: escalationError } = await supabase
        .from('ticket_escalations')
        .select('*')
        .eq('id', escalationId)
      ...
      // Actualizar estado de la acciÃ³n
  ...
  private async executeAssignAction(ticketData: TicketData, parameters: any): Promise<boolean> {
    try {
      ...
      if (newAssignee) {
        ...
        // Registrar la nueva asignaciÃ³n
        const { error: assignmentError } = await supabase
          ...
          .insert({
            ...
            reason: 'ReasignaciÃ³n por escalaciÃ³n',
  ...
  // Ejecutar acciÃ³n de actualizaciÃ³n de etapa
  ...
  // Ejecutar acciÃ³n de actualizaciÃ³n de prioridad
  ...
  // Ejecutar acciÃ³n de creaciÃ³n de tarea
  ...
  // Ejecutar acciÃ³n de envÃ­o de email
  private async executeSendEmailAction(ticketData: TicketData, parameters: any): Promise<boolean> {
    try {
      const emailData = {
        ...
        subject: parameters.subject || `Ticket ${ticketData.id} - ActualizaciÃ³n`,
        template: parameters.template || 'escalation_notification',
        data: {
          ticketId: ticketData.id,
          subject: ticketData.subject,
          priority: ticketData.priority,
          escalationReason: parameters.reason || 'EscalaciÃ³n automÃ¡tica'
        }
      };

      // AquÃ­ se enviarÃ­a el email usando el servicio de email
  ...
  // Enviar notificaciones de escalaciÃ³n
  private async sendEscalationNotifications(ticketData: TicketData, rule: EscalationRule): Promise<void> {
    try {
      ...
      // Notificar a supervisores si estÃ¡ configurado
      if (rule.targetUsers && rule.targetUsers.length > 0) {
        await notificationService.sendNotification({
          type: 'info',
          title: 'Nuevo Ticket Escalado',
          message: `Ticket ${ticketData.id} requiere atenciÃ³n: ${rule.name}`,
  ...
  // Evaluar condiciÃ³n de escalaciÃ³n
  ...
  // Obtener estadÃ­sticas de escalaciÃ³n
