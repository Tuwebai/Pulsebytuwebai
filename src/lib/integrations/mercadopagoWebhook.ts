...
class MercadoPagoWebhookHandler {
  ...
  async processWebhook(webhookData: MercadoPagoWebhookData): Promise<WebhookResult> {
    try {
      ...
      if (!paymentInfo) {
        return {
          success: false,
          message: `No se pudo obtener informaciÃ³n del pago ${paymentId}`,
          processed: false
        };
      }
    ...
    try {
      ...
      const { error: insertError } = await supabase
        ...
        user_name: mercadopagoData.payer?.name || 'Usuario',
          ...
          paymentMethod: mercadopagoData.payment_method?.type,
          installments: mercadopagoData.installments,
          description: mercadopagoData.description || 'Pago recibido vÃ­a webhook',
  ...
  // Verificar autenticidad del webhook (en producciÃ³n)
  async verifyWebhookSignature(payload: string, signature: string): Promise<boolean> {
    try {
      // En producciÃ³n, implementar verificaciÃ³n de firma HMAC
