...
class MercadoPagoSyncService {
  ...
  // Determinar tipo de pago basado en la descripciÃ³n
  ...
  // Determinar caracterÃ­sticas del plan
  private determineFeatures(mpPayment: MercadoPagoPayment): string[] {
    ...
    if (description.includes('basic')) {
      return ['5 proyectos', 'Soporte por email', 'Funciones bÃ¡sicas'];
    }
  ...
  async testConnection(): Promise<boolean> {
    try {
      ...
      console.error('Error probando conexiÃ³n con MercadoPago:', error);
      return false;
    }
  }
  ...
  async getSyncStats(userEmail: string): Promise<{
    ...
    try {
      ...
      // Si hay error de RLS o permisos, retornar estadÃ­sticas vacÃ­as
      if (error) {
        console.warn('Error obteniendo pagos (posiblemente RLS o sin permisos):', error);
        // Retornar estadÃ­sticas vacÃ­as en lugar de lanzar error
        return {
          totalPayments: 0,
          syncedPayments: 0,
          pendingPayments: 0,
  ...
  async syncPayments(userEmail: string): Promise<SyncResult> {
    try {
      ...
      const message = success 
        ? `SincronizaciÃ³n completada. ${syncedCount} pagos sincronizados exitosamente.`
        : `SincronizaciÃ³n parcial. ${syncedCount} pagos sincronizados, ${errors.length} errores.`;
      ...
      console.error('Error en sincronizaciÃ³n de pagos:', error);
      return {
        success: false,
        syncedPayments: 0,
        errors: [error.message],
        message: 'Error durante la sincronizaciÃ³n de pagos'
      };
    }
  }
}
