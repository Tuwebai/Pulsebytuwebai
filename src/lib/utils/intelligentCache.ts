...
export interface CacheEntry<T> {
  ...
  key: string;
  tags?: string[]; // Para invalidaciÃ³n por tags
}

export interface CacheConfig {
  defaultTTL: number; // TTL por defecto en ms
  maxSize: number; // TamaÃ±o mÃ¡ximo del cachÃ©
...
export class IntelligentCache {
  ...
  constructor(config: Partial<CacheConfig> = {}) {
    this.config = {
      ...
      maxSize: 1000, // 1000 entradas mÃ¡ximo
  ...
  // FunciÃ³n con cachÃ© automÃ¡tico
  async withCache<T>(
    key: string,
    fetcher: () => Promise<T>,
    ttl?: number,
    ...
    // Si no estÃ¡ en cachÃ©, obtener datos
    try {
  ...
  // Evictar la entrada mÃ¡s antigua
  ...
  private calculateHitRate(): number {
    // ImplementaciÃ³n simplificada - en producciÃ³n usar mÃ©tricas reales
    return 0.85; // 85% de aciertos estimado
  }
}

// Instancias de cachÃ© especializadas
...
export const chartDataCache = new IntelligentCache({
  defaultTTL: 30 * 60 * 1000, // 30 minutos para datos de grÃ¡ficos
  maxSize: 100
});

// FunciÃ³n helper para generar claves de cachÃ©
