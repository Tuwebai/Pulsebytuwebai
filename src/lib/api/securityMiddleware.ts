...
export interface SecurityConfig {
  ...
  session: {
    maxAge: number; // DuraciÃ³n mÃ¡xima de sesiÃ³n en ms
    refreshThreshold: number; // Tiempo antes de expirar para refrescar
    requireReauth: boolean; // Requerir reautenticaciÃ³n periÃ³dica
  };
  csrf: {
    enabled: boolean;
    tokenLength: number;
...
export class SecurityMiddleware {
  ...
  async verifyAuthentication(): Promise<{ valid: boolean; user?: any; needsRefresh?: boolean }> {
    try {
      ...
      handleSupabaseError(error, 'Verificar autenticaciÃ³n');
      return { valid: false };
    }
  }
  ...
  private startCleanup(): void {
    setInterval(() => {
      ...
      this.securityEvents = this.securityEvents.filter(event => 
        (now - event.timestamp.getTime()) < (24 * 60 * 60 * 1000) // Ãšltimas 24 horas
      );
  ...
  getSecurityStats(): {
    ...
    const recentEvents = this.securityEvents.filter(event => 
      (Date.now() - event.timestamp.getTime()) < (60 * 60 * 1000) // Ãšltima hora
    );
