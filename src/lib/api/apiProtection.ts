...
import { validateQueryParams, sanitizeForDatabase } from '../utils/validation';
import { sessionManager } from './sessionManager';

// =====================================================
// CONFIGURACIÃ“N DE PROTECCIÃ“N API
// =====================================================
...
// =====================================================
// CLASE PRINCIPAL DE PROTECCIÃ“N API
// =====================================================
...
export class APIProtection {
      ...
      try {
        ...
        if (request.method !== config.method) {
          return this.createErrorResponse(
            'Method Not Allowed',
            405,
            `MÃ©todo ${request.method} no permitido para ${config.path}`,
            requestId
          );
        }
        ...
        // 3. Verificar autenticaciÃ³n si es requerida
        ...
        // 5. Validar parÃ¡metros de consulta
  ...
  private async checkRateLimit(
    ...
    if (current.count >= rateLimit.maxRequests) {
      return this.createErrorResponse(
        'Too Many Requests',
        429,
        `Rate limit excedido. MÃ¡ximo ${rateLimit.maxRequests} requests por ${rateLimit.windowMs / 60000} minutos`,
        this.generateRequestId()
      );
    }
  ...
  /**
   * Valida parÃ¡metros de consulta
   */
  private validateQueryParameters(request: AuthenticatedRequest): NextResponse | null {
    ...
    if (!validation.isValid) {
      return this.createErrorResponse(
        'Bad Request',
        400,
        `ParÃ¡metros de consulta invÃ¡lidos: ${validation.errors.join(', ')}`,
        this.generateRequestId()
      );
    }
    ...
    try {
      ...
      if (!validation.success) {
        ...
        return this.createErrorResponse(
          'Bad Request',
          400,
          `Cuerpo de solicitud invÃ¡lido: ${errors.join(', ')}`,
          this.generateRequestId()
        );
      }
  ...
  private checkTimeout(startTime: number, timeoutMs: number): NextResponse | null {
    ...
    if (elapsed > timeoutMs) {
      return this.createErrorResponse(
        'Request Timeout',
        408,
        `La solicitud excediÃ³ el tiempo lÃ­mite de ${timeoutMs}ms`,
        this.generateRequestId()
      );
    }
  ...
  private logRequest(
    ...
    // Mantener solo los Ãºltimos 100 logs por endpoint
    if (logs.length > 100) {
    ...
    if (API_CONFIG.LOGGING.LOG_FAILURES && status === 'ERROR') {
      console.error(`âŒ API ${context.method} ${context.endpoint} - Error: ${error?.message}`);
    }
    
    if (API_CONFIG.LOGGING.LOG_SLOW_REQUESTS && duration > API_CONFIG.LOGGING.SLOW_REQUEST_THRESHOLD_MS) {
      console.warn(`ðŸŒ API ${context.method} ${context.endpoint} - Lenta: ${duration}ms`);
    }
  }

  /**
   * Obtiene estadÃ­sticas de requests
   */
  ...
  /**
   * Obtiene logs de un endpoint especÃ­fico
   */
...
export function validateCSRFToken() {
  return (request: AuthenticatedRequest): NextResponse | null => {
    ...
    // AquÃ­ implementarÃ­as la validaciÃ³n real del token CSRF
