...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-[var(--border-subtle)] bg-[var(--bg-surface)] pb-[env(safe-area-inset-bottom)] md:hidden" data-tour="shell-bottom-nav">
        ...
        {bottomNavItems.map(({ icon: Icon, label, to }) => (
          <li key={to} className="flex-1">
            <NavLink
              ...
              data-tour={`shell-mobile-nav-${label.toLowerCase().replace(/\s+/g, '-')}`}
              onClick={() => {
