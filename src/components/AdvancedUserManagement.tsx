...
export default function AdvancedUserManagement() {
  ...
  const getStatusColor = (status: UserStatus | 'pending' | string) => {
    switch (status) {
  ...
  const getStatusIcon = (status: UserStatus | 'pending' | string) => {
    switch (status) {
  ...
  const filteredUsers = users.filter(user => {
    const matchesSearch = user.email.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         (user.display_name ?? user.full_name ?? '').toLowerCase().includes(searchTerm.toLowerCase());
                ...
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      <Avatar>
                        <AvatarImage src={user.avatar ?? user.avatar_url ?? undefined} />
                        <AvatarFallback>
                      ...
                      <div>
                        <h3 className="font-medium">{user.display_name ?? user.full_name ?? 'Sin nombre'}</h3>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          <Badge className={getStatusColor(user.status ?? 'inactive')}>
                            ...
                            <span className="ml-1">{user.status ?? 'inactive'}</span>
                          </Badge>
                        </div>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <span className="text-xs text-muted-foreground">
                          Expira: {new Date(invitation.expiresAt ?? invitation.expires_at).toLocaleDateString('es-ES')}
                        </span>
                      </div>
                    </div>
                ...
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <span className="text-xs text-muted-foreground">
                        {new Date(log.timestamp ?? log.created_at).toLocaleString('es-ES')}
                      </span>
                    </div>
