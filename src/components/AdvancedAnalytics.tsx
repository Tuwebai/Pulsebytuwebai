...
export default function AdvancedAnalytics() {
  ...
  const loadAnalyticsData = async () => {
    ...
    try {
      ...
      const [projectsResponse, usersResponse, paymentsResponse, ticketsResponse] = await Promise.all([
        supabase.from('projects').select('id, name, created_at, status, type, completed_at'),
        supabase.from('users').select('id, full_name, email, created_at, last_login'),
        user?.role === 'admin' 
      ...
      const projects: AnalyticsProjectRow[] = projectsResponse.data || [];
      const users: AnalyticsUserRow[] = usersResponse.data || [];
      const payments: AnalyticsPaymentRow[] = paymentsResponse.data || [];
      const tickets: AnalyticsTicketRow[] = ticketsResponse.data || [];
      ...
      const activeUsers = users.filter((currentUser) => {
        const lastLogin = getDateValue(currentUser.last_login);
        return lastLogin !== null && lastLogin > thirtyDaysAgo;
      }).length;
      ...
      const averageCompletionTime = completedProjects > 0 ? 
        projects.filter(p => p.status === 'completed' && p.completed_at)
          .reduce((sum, p) => {
            const created = getDateValue(p.created_at);
            const completed = getDateValue(p.completed_at);
  ...
  const generateRealTimeSeriesData = (
    ...
    for (let i = 29; i >= 0; i--) {
      ...
      const projectsCount = projects.filter(p => {
        ...
        return createdAt !== null && createdAt.toDateString() === date.toDateString();
      }).length;
      
      // Contar usuarios registrados en esta fecha
      const usersCount = users.filter(u => {
        const createdAt = getDateValue(u.created_at);
        return createdAt !== null && createdAt.toDateString() === date.toDateString();
      }).length;
      ...
      const tasksCount = tickets.filter(t => {
        const createdAt = getDateValue(t.created_at);
        return createdAt !== null && createdAt.toDateString() === date.toDateString();
      }).length;
  ...
  const generateRealUserActivityData = (users: AnalyticsUserRow[]): AnalyticsData['userActivity'] => {
    const data: AnalyticsData['userActivity'] = [];
    const now = new Date();
    const thirtyDaysAgo = new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000);
  ...
  const generateRealRevenueData = (
    payments: AnalyticsPaymentRow[],
    projects: AnalyticsProjectRow[]
  ): AnalyticsData['revenueByMonth'] => {
    const data: AnalyticsData['revenueByMonth'] = [];
