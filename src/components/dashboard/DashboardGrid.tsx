...
interface StoredDashboardView extends Omit<DashboardView, 'createdAt' | 'updatedAt'> {
  createdAt: string;
  updatedAt: string;
}
...
const DashboardGrid: React.FC<DashboardGridProps> = ({
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        const parsedViews = JSON.parse(saved) as StoredDashboardView[];
        // Update timestamps
        const viewsWithDates = parsedViews.map((view) => ({
          ...view,
