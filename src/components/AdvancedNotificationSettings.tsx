...
type NotificationCategories = NotificationPreferences['channels']['email']['settings']['categories'];
type TimedChannelKey = Exclude<ChannelKey, 'inApp'>;
type TimedChannelSettings = NotificationPreferences['channels']['email']['settings'];
type InAppChannelSettings = NotificationPreferences['channels']['inApp']['settings'];
type ChannelConfig = NotificationPreferences['channels'][ChannelKey];

const getServiceChannelName = (channel: ChannelKey | string): string => {
  return channel === 'inApp' ? 'in_app' : channel;
};

const isTimedChannel = (channelKey: ChannelKey): channelKey is TimedChannelKey => {
  return channelKey !== 'inApp';
};

const testNotificationChannel = async (channel: ChannelKey | string): Promise<boolean> => {
  return notificationChannelService.getChannel(getServiceChannelName(channel)) !== undefined;
};
...
export default function AdvancedNotificationSettings() {
  ...
  const checkChannelStatuses = async () => {
    try {
      ...
      for (const channel of channels) {
        try {
          const isConnected = await testNotificationChannel(channel);
          statuses.push({
            channel,
  ...
  const updateChannelPreference = <TChannel extends ChannelKey>(
    channel: TChannel,
    updates: Partial<NotificationPreferences['channels'][TChannel]>
  ) => {
  ...
  const updateTimedChannelSettings = <TChannel extends TimedChannelKey>(
    channel: TChannel,
    settings: NotificationPreferences['channels'][TChannel]['settings']
  ) => {
    switch (channel) {
      case 'email':
        updateChannelPreference('email', { settings: settings as NotificationPreferences['channels']['email']['settings'] });
        return;
      case 'push':
        updateChannelPreference('push', { settings: settings as NotificationPreferences['channels']['push']['settings'] });
        return;
      case 'sms':
        updateChannelPreference('sms', { settings: settings as NotificationPreferences['channels']['sms']['settings'] });
        return;
    }
  };

  const updateInAppChannelSettings = (
    settings: NotificationPreferences['channels']['inApp']['settings']
  ) => {
    updateChannelPreference('inApp', { settings });
  };

  const updateChannelCategories = <TChannel extends ChannelKey>(
    channel: TChannel,
    categories: NotificationPreferences['channels'][TChannel]['settings']['categories']
  ) => {
    switch (channel) {
      case 'email':
        updateChannelPreference('email', {
          settings: {
            ...preferences!.channels.email.settings,
            categories
          }
        });
        return;
      case 'push':
        updateChannelPreference('push', {
          settings: {
            ...preferences!.channels.push.settings,
            categories
          }
        });
        return;
      case 'sms':
        updateChannelPreference('sms', {
          settings: {
            ...preferences!.channels.sms.settings,
            categories
          }
        });
        return;
      case 'inApp':
        updateChannelPreference('inApp', {
          settings: {
            ...preferences!.channels.inApp.settings,
            categories
          }
        });
        return;
    ...
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      <Tabs defaultValue="channels" className="space-y-6">
        ...
        <TabsContent value="channels" className="space-y-6">
          {Object.entries(preferences.channels).map(([rawChannelKey, rawChannelConfig]) => {
            const channelKey = rawChannelKey as ChannelKey;
            const channelConfig = rawChannelConfig as ChannelConfig;
                ...
                <CardTitle className="flex items-center justify-between">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(enabled) => 
                      updateChannelPreference(channelKey, { enabled })
                    }
                  />
                </CardTitle>
              </CardHeader>
              {channelConfig.enabled && (
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {isTimedChannel(channelKey) && (() => {
                      const timedSettings = channelConfig.settings as NotificationPreferences['channels'][typeof channelKey]['settings'];

                      return (
                      <div>
                        <Label>Frecuencia</Label>
                        <Select
                          value={timedSettings.frequency}
                          onValueChange={(frequency) => 
                            updateTimedChannelSettings(channelKey, {
                              ...timedSettings,
                              frequency: frequency as NotificationPreferences['channels'][typeof channelKey]['settings']['frequency']
                            })
                          }
                        >
                          <SelectTrigger>
                          ...
                          <SelectContent>
                            ...
                            {(channelKey === 'email' || channelKey === 'sms') && <SelectItem value="daily">Diario</SelectItem>}
                            {channelKey === 'email' && <SelectItem value="weekly">Semanal</SelectItem>}
                            {channelKey === 'push' && <SelectItem value="batched">Agrupado</SelectItem>}
                          </SelectContent>
                        </Select>
                      </div>
                      );
                    ...
                    {channelKey === 'inApp' && (() => {
                      const inAppSettings = channelConfig.settings as InAppChannelSettings;

                      return (
                      <div>
                        <Label>Ocultar autom??ticamente (ms)</Label>
                        <Input
                          type="number"
                          value={inAppSettings.autoHideDelay}
                          onChange={(e) => 
                            updateInAppChannelSettings({
                              ...inAppSettings,
                    ...
                    <div className="flex gap-4">
                      {Object.entries(preferences.channels).map(([rawChannelKey, rawChannelConfig]) => {
                        ...
                        <div key={channelKey} className="flex items-center gap-1">
                          ...
                          <Switch
                            checked={channelConfig.settings.categories[category as keyof NotificationCategories]}
                            onCheckedChange={(checked) => 
                              updateChannelCategories(channelKey, {
              ...
              {Object.entries(preferences.channels).map(([rawChannelKey, rawChannelConfig]) => {
                ...
                <div key={channelKey} className="p-4 border rounded-lg">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <Switch
                      checked={timedSettings.quietHours.enabled}
                      onCheckedChange={(enabled) => 
                        updateTimedChannelSettings(channelKey, {
                          ...timedSettings,
                          quietHours: { ...timedSettings.quietHours, enabled }
                        })
                      }
                    />
                  </div>
                  
                  {timedSettings.quietHours.enabled && (
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <Label>Desde</Label>
                        <Input
                          type="time"
                          value={timedSettings.quietHours.start}
                          onChange={(e) => 
                            updateTimedChannelSettings(channelKey, {
                              ...timedSettings,
                              quietHours: { ...timedSettings.quietHours, start: e.target.value }
                            })
                          }
                        />
                      </div>
                      <div>
                        ...
                        <Input
                          type="time"
                          value={timedSettings.quietHours.end}
                          onChange={(e) => 
                            updateTimedChannelSettings(channelKey, {
                              ...timedSettings,
                              quietHours: { ...timedSettings.quietHours, end: e.target.value }
                            })
                          }
                        />
                      </div>
