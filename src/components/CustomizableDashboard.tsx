...
import type { DashboardLayout as CustomDashboardLayout, DashboardWidget as Widget } from '@/lib/services/userPreferencesService';
import { reportService } from '@/lib/services/reportService';

type ChartType = NonNullable<Widget['config']['chartType']>;
...
export default function CustomizableDashboard() {
  ...
  const saveDashboardLayouts = useCallback(async (layoutsToSave: CustomDashboardLayout[]) => {
    try {
      // Guardar en localStorage como fallback
      localStorage.setItem('dashboardLayouts', JSON.stringify(layoutsToSave));
      
      // Guardar en base de datos si el usuario est?? autenticado
    ...
    if (isAuthenticated && user && !layoutsLoaded) {
      ...
      try {
        ...
        if (saved) {
          ...
          const normalizedLayouts = userPreferencesService.normalizeDashboardLayouts(parsed);
          setLayouts(normalizedLayouts);
          const defaultLayout = normalizedLayouts.find((l) => l.isDefault);
          setCurrentLayout(defaultLayout || normalizedLayouts[0] || null);
        } else {
          // Crear layout por defecto
          const defaultLayout: CustomDashboardLayout = {
            id: 'default',
  ...
  }, [isAuthenticated, layoutsLoaded, saveDashboardLayouts, user]);

  useEffect(() => {
    void loadDashboardLayouts();
            ...
            {newWidget.type === 'chart' && (
              <>
                ...
                <div>
                  ...
                  <Select value={newWidget.config?.chartType} onValueChange={(value) => setNewWidget({ ...newWidget, config: { ...newWidget.config, chartType: value as ChartType } })}>
                    <SelectTrigger>
                      <SelectValue placeholder="Seleccionar tipo" />
                    </SelectTrigger>
                    <SelectContent>
