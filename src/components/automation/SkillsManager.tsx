...
import { useSkills, useCreateSkill, useUpdateSkill, useDeleteSkill, type UserSkill } from '@/hooks/useSkills';
...
export const SkillsManager: React.FC<SkillsManagerProps> = ({ userId }) => {
  ...
  const handleDeleteSkill = async (skillId: string) => {
    try {
      if (userId) {
        await deleteSkill.mutateAsync({ id: skillId, userId });
      } else {
        // Fallback al método original
      ...
      // React Query maneja invalidación automáticamente a través de useSkills
      queryClient.invalidateQueries({ queryKey: ['skills', userId] });
      // Recargar análisis de habilidades
      const analysis = await getSkillAnalysis();
      setSkillAnalysis(analysis);
    } catch (error) {
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center space-x-4 text-sm text-muted-foreground">
                      ...
                      <span className="flex items-center">
                        ...
                        Último uso: {new Date(skill.last_used ?? skill.updated_at ?? skill.created_at ?? new Date().toISOString()).toLocaleDateString()}
                      </span>
                    </div>
                    <div className="mt-2">
                      <Progress 
