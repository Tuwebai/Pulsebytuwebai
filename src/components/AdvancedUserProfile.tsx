...
export default function AdvancedUserProfile() {
  ...
  const loadUserProfile = async () => {
    ...
    try {
      const [profileData, auditData] = await Promise.all([
        userManagementService.getUserById(user.id),
        userManagementService.getAuditLogs(user.id, 50)
      ]);
  ...
  const getActivityStats = () => {
    ...
    const todayActions = auditLogs.filter(log => {
      const logDate = new Date(log.timestamp ?? log.created_at).toDateString();
      const today = new Date().toDateString();
      return logDate === today;
    }).length;

    const uniqueResources = new Set(auditLogs.map(log => log.resource ?? 'system')).size;
    const mostUsedAction = auditLogs.reduce((acc, log) => {
      acc[log.action] = (acc[log.action] || 0) + 1;
      return acc;
    }, {} as Record<string, number>);

    const topAction = (Object.entries(mostUsedAction) as Array<[string, number]>).sort(([, aCount], [, bCount]) => bCount - aCount)[0];
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Avatar className="h-20 w-20">
            <AvatarImage src={profile.avatar ?? profile.avatar_url ?? undefined} />
                  ...
                  <div key={log.id} className="flex items-center justify-between p-3 border rounded-lg">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        Recurso: {log.resource ?? 'system'} ? {new Date(log.timestamp ?? log.created_at).toLocaleString('es-ES')}
                      </p>
                    </div>
              ...
              <CardContent className="space-y-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {profileMetadata.lastLogin 
                        ? new Date(profileMetadata.lastLogin).toLocaleString('es-ES')
                        : 'Nunca'
                      }
                    </p>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">{profileMetadata.loginCount}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {new Date(profileMetadata.createdAt).toLocaleDateString('es-ES')}
                    </p>
                  </div>
                </div>
              </CardContent>
