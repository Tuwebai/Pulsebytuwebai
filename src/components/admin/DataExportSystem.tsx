...
type ExportJobDraft = Omit<Partial<ExportJob>, 'filters'> & {
  filters: ExportFilters;
};
...
export default function DataExportSystem() {
  ...
  const [newExport, setNewExport] = useState<ExportJobDraft>({
    name: '',
    description: '',
    dataType: 'projects',
    format: 'excel',
    ...
    try {
      ...
      setTimeout(async () => {
        try {
          ...
          setExportJobs(prev => prev.map(job => 
            job.id === exportJob.id 
              ? { ...job, status: 'failed', completedAt: new Date(), error: errorMessage }
              : job
          ));
  ...
  const processExport = async (data: unknown[], format: ExportJob['format']) => {
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Select 
                  ...
                  onValueChange={(value: ExportJob['dataType']) => setNewExport(prev => ({ ...prev, dataType: value }))}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Select 
                  ...
                  onValueChange={(value: ExportJob['format']) => setNewExport(prev => ({ ...prev, format: value }))}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
            ...
            <div className="flex items-center space-x-2">
              <Checkbox 
                ...
                onCheckedChange={(checked) => setNewExport(prev => ({ 
                  ...prev, 
                  filters: { ...prev.filters, includeArchived: checked === true }
                }))}
              />
            ...
            <div className="flex items-center space-x-2">
              <Checkbox 
                ...
                onCheckedChange={(checked) => setNewExport(prev => ({ 
                  ...prev, 
                  filters: { ...prev.filters, includeDeleted: checked === true }
                }))}
              />
