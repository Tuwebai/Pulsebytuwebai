...
import { Project, CreateProjectData, UpdateProjectData, EnvironmentVariable as ProjectEnvironmentVariable } from '@/types/project.types';
import { environmentService, type EnvironmentVariable as StoredEnvironmentVariable } from '@/lib/environmentService';
...
export const ProjectForm: React.FC<ProjectFormProps> = ({
  ...
  const [envVars, setEnvVars] = useState<ProjectEnvironmentVariable[]>([]);
    ...
    const loadProjectData = async () => {
      if (project) {
        ...
        try {
          ...
          setEnvVars(envVarsFromDB.map(({ key, value, isSensitive }) => ({ key, value, isSensitive })));
        } catch (error) {
          console.error('Error loading environment variables:', error);
          // Fallback a variables del proyecto si las hay
          const envArray: ProjectEnvironmentVariable[] = [];
  ...
  const handleInputChange = <K extends keyof CreateProjectData>(field: K, value: CreateProjectData[K]) => {
  ...
  const handleGitHubUrlBlur = async () => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : 'Error desconocido al analizar el repositorio';
      ...
      } else if (message.includes('URL de GitHub inv?lida')) {
        errorMessage = 'URL de GitHub no v?lida';
      } else if (message.includes('Token de GitHub inv?lido')) {
  ...
  const applyDetectedInfo = () => {
    ...
    const detectedEnvVars: ProjectEnvironmentVariable[] = Object.entries(detectedInfo.environment_variables || {}).map(([key, value]) => ({
      key,
      value,
      isSensitive: false
    }));
  ...
  const addEnvironmentVariable = () => {
    ...
    const newEnv: ProjectEnvironmentVariable = {
      key: '',
      value: '',
      isSensitive: false
    };
  ...
  const handleKeyPaste = (e: React.ClipboardEvent, index: number) => {
    ...
    if (pastedText.includes('\n')) {
      ...
      const newVars: ProjectEnvironmentVariable[] = [];
    ...
    if (project?.id) {
      try {
        ...
        for (const envVar of validEnvVars) {
          const environmentVariable: StoredEnvironmentVariable = {
            ...envVar,
            project_id: project.id,
            environment: 'production'
          };

          await environmentService.upsertEnvironmentVariable(environmentVariable);
        }
      } catch (error) {
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(value: CreateProjectData['status']) => handleInputChange('status', value)}
            >
                ...
                <div>
                  ...
                  >
                    {detectedInfo.status === 'development' ? 'Desarrollo' :
                     ...
                     detectedInfo.status === 'maintenance' ? 'Mantenimiento' : detectedInfo.status}
                  </Badge>
                </div>
