...
const mapReportExecution = (row: ReportExecutionRow): ReportExecution => ({
  id: row.id,
...
export default function AdvancedReportsManager() {
  ...
  const loadReportData = async () => {
    try {
      ...
      setExecutions((executionsData || []).map((execution) => mapReportExecution(execution as ReportExecutionRow)));
  ...
  const generateReport = async (template: ReportTemplate) => {
    try {
      ...
      setExecutions((prev) => [mapReportExecution(execution as ReportExecutionRow), ...prev]);
      ...
      setTimeout(async () => {
        try {
          ...
          setExecutions(prev => prev.map(e => 
            e.id === execution.id 
              ? { ...e, status: 'completed', completedAt: new Date(), downloadUrl }
              : e
          ));
          ...
          setExecutions(prev => prev.map(e => 
            e.id === execution.id 
              ? { ...e, status: 'failed', completedAt: new Date(), error: error instanceof Error ? error.message : 'Error desconocido' }
              : e
          ));
  ...
  const exportReport = async (_data: unknown, type: ReportTemplate['type']) => {
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-medium">
                          {templates.find(t => t.id === execution.templateId)?.name || 'Template no encontrado'}
                        </p>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Select 
                  ...
                  onValueChange={(value: ReportTemplate['type']) => setNewTemplate(prev => ({ ...prev, type: value }))}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
