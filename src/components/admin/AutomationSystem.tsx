...
import { workflowService, type Workflow } from '@/lib/workflowService';
import { triggerService, type SystemTrigger } from '@/lib/triggerService';
import { automationTaskService, type AutomationTask } from '@/lib/automationTaskService';
...
type TriggerView = SystemTrigger & {
  status?: 'active' | 'inactive';
};

type AutomationTaskView = AutomationTask & {
  frequency?: string;
...
export default function AutomationSystem() {
  ...
  const [stats, setStats] = useState<AutomationStats>(DEFAULT_AUTOMATION_STATS);
  ...
  const getAutomationStats = async () => {
    try {
      ...
      return DEFAULT_AUTOMATION_STATS;
    }
  };
       ...
       <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="overview" className="space-y-6">
            ...
            <CardContent>
              {automationLogs.length === 0 ? (
                ...
                <div className="space-y-3">
                  {automationLogs.slice(0, 5).map((workflow) => (
          ...
          <Card className="bg-white border-slate-200">
            ...
            <CardContent>
              {tasks.filter((task) => task.next_execution).length === 0 ? (
                <div className="text-center py-8">
                ...
                <div className="space-y-2">
                  {tasks
                    .filter((task): task is AutomationTaskView & { next_execution: string } => Boolean(task.next_execution))
                    .slice(0, 5)
                    .map((task) => (
...
// Pestaña de Resumen
function OverviewTab({ workflows, triggers, tasks, stats }: OverviewTabProps) {
  return (
    <div className="space-y-6">
             {/* Actividad Reciente */}
       <Card className="bg-white border-slate-200">
...
function WorkflowsTab({ workflows, onRefresh, onShowWorkflowForm, onDeleteWorkflow }: WorkflowsTabProps) {
...
// Pestaña de Triggers
function TriggersTab({ triggers, onRefresh, onShowTriggerForm, onDeleteTrigger }: TriggersTabProps) {
...
// Pestaña de Tareas
function TasksTab({ tasks, onRefresh, onShowTaskForm, onDeleteTask }: TasksTabProps) {
...
function WorkflowForm({ onClose, onSuccess }: ModalCallbacks) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: 'Error',
        description: getErrorMessage(error, 'No se pudo crear el workflow'),
        variant: 'destructive'
      });
    }
  };
...
function TriggerForm({ onClose, onSuccess }: ModalCallbacks) {
  const [formData, setFormData] = useState({
    name: '',
    description: '',
    event_type: 'project_created' as SystemTrigger['event_type'],
    is_active: true
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: 'Error',
        description: getErrorMessage(error, 'No se pudo crear el trigger'),
        variant: 'destructive'
      });
    }
  };
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value: SystemTrigger['event_type']) => setFormData({ ...formData, event_type: value })}
              >
                <SelectTrigger className="bg-white border-slate-200 text-slate-800">
                ...
                <SelectContent className="bg-white border-slate-200">
                  ...
                  <SelectItem value="project_status_changed">Estado de Proyecto</SelectItem>
                  <SelectItem value="project_deadline_approaching">Vencimiento de Proyecto</SelectItem>
...
// Formulario de Tarea
function TaskForm({ onClose, onSuccess }: ModalCallbacks) {
  const [formData, setFormData] = useState({
    name: '',
    description: '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: 'Error',
        description: getErrorMessage(error, 'No se pudo crear la tarea'),
        variant: 'destructive'
      });
    }
  };
