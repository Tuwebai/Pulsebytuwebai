...
const getRealtimePaymentAmount = (payload: RealtimePostgresChangesPayload<Record<string, unknown>>): number => {
  const nextValue = payload.new;

  if (nextValue && typeof nextValue === 'object' && 'amount' in nextValue) {
    return Number(nextValue.amount) || 0;
  }
...
export default function RealTimeMetrics() {
  ...
  const loadInitialData = async () => {
    try {
      ...
      if (alertsData) {
        setAlerts((alertsData as SystemAlertRow[]).map(alert => ({
          ...alert,
  ...
  const generateInitialMetrics = async (): Promise<RealTimeMetric[]> => {
    ...
    const [projectsResponse, usersResponse, paymentsResponse, ticketsResponse] = await Promise.all([
      supabase.from('projects').select('id, name, created_at, status'),
      supabase.from('users').select('id, full_name, email, created_at'),
      supabase.from('payments').select('id, amount, created_at, status'),
      supabase.from('tickets').select('id, asunto, created_at, status')
    ]);

    const projects: ProjectMetricsRow[] = projectsResponse.data || [];
    const users: UserMetricsRow[] = usersResponse.data || [];
    const payments: PaymentMetricsRow[] = paymentsResponse.data || [];
    const tickets: TicketMetricsRow[] = ticketsResponse.data || [];
  ...
  const generateInitialKPIs = async (): Promise<KPITracking[]> => {
    ...
    const { data: projects } = await supabase.from('projects').select('id, name, created_at, status');
    const { data: payments } = await supabase.from('payments').select('id, amount, created_at, status');
  ...
  const updateMetricsFromRealtime = (payload: RealtimePostgresChangesPayload<Record<string, unknown>>) => {
    ...
    setMetrics(prev => prev.map(metric => {
      ...
      if (metric.id === 'revenue-hour' && payload.table === 'payments') {
        return { ...metric, value: metric.value + getRealtimePaymentAmount(payload) };
      }
      return metric;
    }));
