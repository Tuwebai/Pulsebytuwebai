...
interface WorkflowActionItem {
  key: 'request_review' | 'approve' | 'deploy' | 'rollback' | 'health_check';
  label: string;
  description: string;
...
export const WorkflowActions: React.FC<WorkflowActionsProps> = ({ version, onActionComplete }) => {
  ...
  const getAvailableActions = (): WorkflowActionItem[] => {
    ...
    switch (version.status) {
      case 'draft':
        actions.push({
          key: 'request_review',
          label: 'Solicitar Revisi??n',
          description: 'Enviar la versi??n para revisi??n por parte del equipo',
        ...
        actions.push(
          {
            key: 'approve',
            label: 'Aprobar',
            description: 'Aprobar la versi??n para deployment',
          ...
          },
          {
            key: 'request_review',
            label: 'Rechazar',
            description: 'Devolver la versi??n al flujo de revisi??n para ajustes',
        ...
        actions.push(
          {
            key: 'deploy',
            label: 'Desplegar',
            description: 'Iniciar el proceso de deployment',
      ...
      case 'deployed':
        actions.push(
          ...
          },
          {
            key: 'rollback',
            label: 'Rollback',
            description: 'Revertir a la versi??n anterior autom??ticamente',
