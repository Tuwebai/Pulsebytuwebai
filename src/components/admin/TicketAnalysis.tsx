...
interface AnalyzedTicket extends Ticket {
  aiAnalysis: TicketAIAnalysis;
}

const normalizeUrgency = (urgency?: string): TicketAIAnalysis['urgency'] => {
  switch (urgency) {
    case 'critical':
    case 'high':
    case 'medium':
...
interface TicketAnalysisProps {
  tickets: Ticket[];
  onUpdateTicket?: (ticketId: string, updates: { suggestedResponse: string; aiAnalyzed: boolean }) => void;
}
...
export const TicketAnalysis: React.FC<TicketAnalysisProps> = ({
  ...
  const [analyzedTickets, setAnalyzedTickets] = useState<AnalyzedTicket[]>([]);
  const [selectedTicket, setSelectedTicket] = useState<AnalyzedTicket | null>(null);
      ...
      for (const ticket of tickets.slice(-10)) { // Últimos 10 tickets
        ...
        try {
          ...
          analyses.push({
            ...ticket,
            aiAnalysis: {
              urgency: normalizeUrgency(analysis.urgency),
