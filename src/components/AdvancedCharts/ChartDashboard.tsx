...
import { motion, AnimatePresence, OptimizedReorder, OptimizedReorderItem } from '@/components/OptimizedMotion';
...
interface ChartTemplate extends ChartData {
  name: string;
}

// Plantillas de gráficos con datos reales
const getChartTemplates = async (): Promise<ChartTemplate[]> => {
  try {
    const [
      userGrowthData,
      projectStatusData,
...
export default function ChartDashboard({ 
  ...
  const createChartFromTemplate = useCallback(() => {
    ...
    const newChart: ChartConfig = {
      ...
      title: template.name,
      type: template.type,
      data: template.data.map((point) => ({ ...point })),
      options: {},
  ...
  const updateChart = useCallback((chartId: string, nextChart: ChartConfig) => {
    setDashboard(prev => ({
      ...prev,
      charts: prev.charts.map(chart => 
        chart.id === chartId ? nextChart : chart
      )
    }));
  }, []);
    ...
    <div className={`space-y-6 ${className}`}>
      ...
      <AnimatePresence>
        {dashboard.charts.length === 0 ? (
          ...
          <OptimizedReorder className="space-y-6">
            {dashboard.charts.map((chart) => (
              <OptimizedReorderItem key={chart.id} value={chart.id}>
                <motion.div
                ...
                {chartTemplates.map((template) => (
                  ...
                  >
                    <div className="flex items-center space-x-2">
                      ...
                      {template.type === 'scatter' && <BarChart3 className="h-4 w-4" />}
