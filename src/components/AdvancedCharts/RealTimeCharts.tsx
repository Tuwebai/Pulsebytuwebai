...
export default function RealTimeCharts({ className = '' }: RealTimeChartsProps) {
  ...
  const loadSystemMetrics = useCallback(async () => {
    try {
      ...
      const [projectsResponse, usersResponse, paymentsResponse, ticketsResponse] = await Promise.all([
        supabase.from('projects').select('id, name, created_at, updated_at, status, created_by'),
      ...
      const projects: RealTimeProjectRow[] = projectsResponse.data || [];
      const users: RealTimeUserRow[] = usersResponse.data || [];
      const payments: RealTimePaymentRow[] = paymentsResponse.data || [];
      const tickets: RealTimeTicketRow[] = ticketsResponse.data || [];
      ...
      let activeUsers = users.filter(u => {
        ...
        const recentLogin = (() => {
          const lastLogin = getDateValue(u.last_login);
        ...
        const hasRecentProjects = projects.some(p => 
          p.created_by === u.id && new Date(p.created_at) > thirtyDaysAgo
        );
        
        // Verificar si tiene proyectos en cualquier estado (usuario con actividad)
        const hasAnyProjects = projects.some(p => p.created_by === u.id);
  ...
  const validateChartData = useCallback((data: ChartMetricDataItem[], defaultValue: number = 0): ChartMetricDataItem[] => {
    return data.map(item => ({
      ...item,
      value: Number.isNaN(item.value) || item.value === null || item.value === undefined ? defaultValue : Math.max(0, item.value)
    }));
  }, []);
