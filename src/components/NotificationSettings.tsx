...
import { Bell, BellOff, Clock, Volume2, VolumeX, Smartphone, Mail, MessageSquare, Settings } from 'lucide-react';

interface NotificationSettingsProps {
  className?: string;
}

type NotificationSettingsKey = keyof UserNotificationSettings;
type QuietHoursKey = keyof UserNotificationSettings['quietHours'];
type NotificationFrequency = UserNotificationSettings['frequency'];
...
export default function NotificationSettings({ className = '' }: NotificationSettingsProps) {
  ...
  const handleSettingChange = async <K extends NotificationSettingsKey>(key: K, value: UserNotificationSettings[K]) => {
  ...
  const handleQuietHoursChange = async <K extends QuietHoursKey>(key: K, value: UserNotificationSettings['quietHours'][K]) => {
    if (!settings || !user) return;
  ...
  const testNotification = async () => {
    ...
    try {
      await pushNotificationService.sendLocalNotification({
        title: "Pulse - Notificacion de prueba",
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(value) => handleSettingChange('frequency', value as NotificationFrequency)}
              disabled={saving}
            >
              <SelectTrigger>
                <SelectValue />
